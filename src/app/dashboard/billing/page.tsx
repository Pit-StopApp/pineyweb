...
function getNextBillingDate(createdAt: string): string | null {
  const created = new Date(createdAt);
  const day = created.getDate();
  const now = new Date();
  const thisMonth = new Date(now.getFullYear(), now.getMonth(), day);
  if (thisMonth > now) return formatDate(thisMonth.toISOString());
...
export default function Billing() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const { data } = await supabase.from("pineyweb_clients").select("business_name, status, tier, created_at").eq("user_id", session.user.id).single();
  ...
  const activeSince = client?.created_at ? formatDate(client.created_at) : "N/A";
  const nextBilling = isManaged && client?.created_at ? getNextBillingDate(client.created_at) : null;

  return (
    <DashboardShell businessName={client?.business_name} onLogout={handleLogout}>
            ...
            <section className="rounded-xl p-8 border" style={{ backgroundColor: "#f8f3eb", borderColor: "rgba(193,201,191,0.1)" }}>
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-sm italic" style={{ color: "#414942" }}>Active since {activeSince}</p>
                </div>
                <span className="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider text-white" style={{ backgroundColor: "#6e745f" }}>{tier}</span>
              </div>
              <div className="mb-8">
                <h2 className="text-3xl font-extrabold mb-2" style={{ color: "#316342" }}>
                  {isManaged ? "Managed Service" : "One-Time Purchase"}
                </h2>
                {isManaged ? (
                  <p className="text-4xl font-light" style={{ color: "#1d1c17" }}>$99<span className="text-lg" style={{ color: "#414942" }}>/mo</span></p>
                ) : (
                  <p className="text-4xl font-light" style={{ color: "#1d1c17" }}>$799<span className="text-lg" style={{ color: "#414942" }}> one-time</span></p>
                )}
              </div>
              {isManaged && (
                <div className="flex items-center gap-3 py-4 border-t" style={{ borderColor: "rgba(193,201,191,0.2)" }}>
                  ...
                  <div className="text-sm">
                    ...
                    <span className="font-bold ml-1" style={{ color: "#1d1c17" }}>{nextBilling || "N/A"}</span>
                  </div>
                </div>
              )}
            </section>

            {/* Payment Method — only for Managed */}
            {isManaged && (
              <section className="rounded-xl p-8 border" style={{ backgroundColor: "#f8f3eb", borderColor: "rgba(193,201,191,0.1)" }}>
